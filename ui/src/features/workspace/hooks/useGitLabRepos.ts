...
import type { GitLabRepo, GitLabSettings } from "../types"

const API_BASE_URL = import.meta.env.VITE_API_BASE_URL || ""
const buildApiUrl = (path: string) => (API_BASE_URL ? `${API_BASE_URL}${path}` : path)

export function useGitLabSettings() {
  const [settings, setSettings] = React.useState<GitLabSettings | null>(null)
  const [isLoading, setIsLoading] = React.useState(false)

  const fetchSettings = React.useCallback(async () => {
    setIsLoading(true)
    try {
      const res = await fetch(buildApiUrl("/api/gitlab/settings"))
  ...
  const saveSettings = React.useCallback(async (token?: string, url?: string, username?: string) => {
    const res = await fetch(buildApiUrl("/api/gitlab/settings"), {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ token, url, username }),
    })
    if (!res.ok) throw new Error("Failed to save GitLab settings")
    const updated: GitLabSettings = await res.json()
    setSettings(updated)
    return updated
  }, [])
...
export function useGitLabRepos(enabled: boolean) {
  const [repos, setRepos] = React.useState<GitLabRepo[]>([])
  const [isLoading, setIsLoading] = React.useState(false)
  ...
  const fetchRepos = React.useCallback(
    async (nextPage: number, searchTerm: string, reset = false) => {
      ...
      try {
        ...
        const res = await fetch(buildApiUrl(`/api/gitlab/repos?${params}`))
        if (!res.ok) {
          const errData = await res.json().catch(() => null)
          setError(errData?.detail ?? `Failed to load GitLab repos: ${res.status}`)
          return
        }
        ...
        const fetched: GitLabRepo[] = data.repos ?? []
